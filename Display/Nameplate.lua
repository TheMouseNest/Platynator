...
local function GetAurasPoolModern(self)
  ...
  return CreateFramePool("AuraButton", self, "CustomAuraButtonTemplate", function(_, f) f:ClearAllPoints() end, false, function(frame)
    ...
    frame.CountFrame.Count = frame.CountFrame:CreateFontString()
...
function addonTable.Display.NameplateMixin:SetupModernAuras()
  self.BuffDisplay.Wrapped = CreateFrame("AuraContainer", nil, self.BuffDisplay, "CustomAuraContainerTemplate")
  self.BuffDisplay.Wrapped:SetSize(10, 10)
  self.DebuffDisplay.Wrapped = CreateFrame("AuraContainer", nil, self.DebuffDisplay, "CustomAuraContainerTemplate")
  self.DebuffDisplay.Wrapped:SetSize(10, 10)
  self.CrowdControlDisplay.Wrapped = CreateFrame("AuraContainer", nil, self.CrowdControlDisplay, "CustomAuraContainerTemplate")
  self.CrowdControlDisplay.Wrapped:SetSize(10, 10)

  self.AurasPools = {
    buffs = GetAurasPoolModern(self.BuffDisplay.Wrapped),
    debuffs = GetAurasPoolModern(self.DebuffDisplay.Wrapped),
    crowdControl = GetAurasPoolModern(self.CrowdControlDisplay.Wrapped),
  }
end
...
function addonTable.Display.NameplateMixin:InitializeWidgets(design, scaleOffset, scaleMod)
  ...
  if not addonTable.Constants.IsMidnightNext then
    ...
    self:InitializeWidgetsModernAuras(designInfo)
  end
...
function addonTable.Display.NameplateMixin:InitializeWidgetsModernAuras(designInfo)
  ...
  if designInfo.debuffs then
    local t= self.DebuffDisplay.Wrapped:CreateTexture()
    t:SetAllPoints(self.DebuffDisplay.Wrapped)
    ...
    Arrange(self.DebuffDisplay.Wrapped, self.AurasPools.debuffs, designInfo.debuffs)
  end
  ...
  if designInfo.buffs then
    ...
    Arrange(self.BuffDisplay.Wrapped, self.AurasPools.buffs, designInfo.buffs)
  end
  ...
  if designInfo.crowdControl then
    ...
    Arrange(self.CrowdControlDisplay.Wrapped, self.AurasPools.crowdControl, designInfo.crowdControl)
  end
end
